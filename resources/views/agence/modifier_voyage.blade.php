...
@section('css')
<link rel="stylesheet" href="/style/agences/modifier_voyage.css">
@endsection
@section('content')
<main>
<div class="main__container">
<div class="div_ajoute">
    <form action="/agenceModifierVoyage/{{ $voyage->id }}" method="post" class="form_voyage" enctype="multipart/form-data">
        @method('PUT')
        @csrf
      <div>
        <h3 class="form_title"><a href="/nos_voyages">Retourner</a></h3>
      ...
      <div>
        <label>
          Type d'annonce : 
          <select name="type" value="{{ $voyage->type }}">
            <option disabled selected> -- Choisi le type -- </option>
            <option value="Client" @if ($voyage->type == 'Client') selected @endif>Client</option>
            <option value="Agence" @if($voyage->type == 'Agence') selected @endif>Agence</option>
        </select>
        </label>
      </div>
      @if ($errors->has('type'))
      <div class="erreur">{{ $errors->first('type') }}</div>
      @endif
      <br />
      <div>
        <label>
